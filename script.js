...
// ═══════════════════════════════════════════════════════════════
// STATE EXTRA — Top Establecimientos (pestaña nueva) + modal genérico
// ═══════════════════════════════════════════════════════════════
let topEstabCache = [];      // [ [estabKey, {noSat,disponible,requerida,red}] , ... ] ordenado desc
let topEstabOpen  = new Set(); // índices expandidos
let topProdsCache = [];      // cache de renderTopProds() para abrir el modal por índice
...
function buildDateGroupedData() {
  ...
  rows.forEach(r => {
    ...
    if (!map[key]) map[key] = { dateKey: dKey, fecha: r.fecha, redes: r.redes, estab: r.estab, cod_pre: r.cod_pre, cod_sismed: r.cod_sismed, producto: r.producto, servicio: r.servicio, mesKey: r.mesKey, requerida:0, disponible:0, noSat:0, count:0 };
    const it = map[key];
...
function switchTab(tab) {
  ...
  const titles = {
    ...
    nuevos: 'Resumen por Producto · Vista General', topestab: 'Top Establecimientos · Mayor Demanda No Atendida'
  };
...
function renderAll() {
  ...
  renderNuevosTable();
  renderTopEstabTab(filteredData);
}
...
function buildDashHTML() {
  ...
  <div class="tab-page" data-tab="detalle">
    <div class="tbl-card" style="flex:1;min-height:0">
      <div class="tbl-bar">
        ...
        <select class="inp" id="t-mes" onchange="populateFilters();filterTable()" style="max-width:150px"><option value="">Todos los meses</option></select>
  ...
  <!-- TAB: TOP ESTABLECIMIENTOS -->
  <div class="tab-page" data-tab="topestab">
    <div class="tbl-card" style="flex:1;min-height:0">
      <div class="tbl-bar">
        <span class="tbl-bar-title">Establecimientos con Mayor Demanda No Atendida</span>
        <input class="inp" type="text" id="te-search" placeholder="🔍 Buscar establecimiento…" style="width:200px" oninput="renderTopEstabTab(filteredData)">
        <span class="tbl-count" id="te-count"></span>
      </div>
      <div style="padding:12px 14px;flex:1;min-height:0;display:flex;flex-direction:column">
        <div class="topestab-list" id="topestab-list"></div>
      </div>
    </div>
  </div>
...
function renderTopProds(data) {
  ...
  tbody.innerHTML = top.map(([prod, info], i) => {
    ...
    return `<tr style="cursor:pointer" onclick="showProductoCentros(${i})" title="Ver de qué centros proviene">
...
// ─── Modal: de qué centros viene la mayor no atención de un producto ──
function showProductoCentros(i) {
  const entry = topProdsCache[i];
  if (!entry) return;
  const [prod] = entry;
  const porCentro = {};
  filteredData.filter(r => r.producto === prod).forEach(r => {
    if (!porCentro[r.estab]) porCentro[r.estab] = { noSat:0, requerida:0, disponible:0, red:r.redes };
    porCentro[r.estab].noSat      += r.noSat;
    porCentro[r.estab].requerida  += r.requerida;
    porCentro[r.estab].disponible += r.disponible;
  });
  const filas = Object.entries(porCentro).sort((a,b) => b[1].noSat - a[1].noSat);
  const totalNoSat = filas.reduce((s,[,v]) => s+v.noSat, 0);
  const rowsHtml = filas.map(([estab, v], idx) => {
    const cob = v.requerida ? (v.disponible/v.requerida*100) : 0;
    const pillCls = cob===0?'pill-red':cob<30?'pill-orange':cob<100?'pill-yellow':'pill-green';
    return `<tr>
      <td class="rank ${idx===0?'r1':idx===1?'r2':idx===2?'r3':''}">${idx+1}</td>
      <td style="max-width:220px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${estab}</td>
      <td style="color:var(--muted2)">${v.red||'—'}</td>
      <td class="mono" style="text-align:right;font-weight:700;color:var(--red)">${v.noSat.toLocaleString()}</td>
      <td><span class="pill ${pillCls}">${cob.toFixed(1)}%</span></td>
    </tr>`;
  }).join('');
  const body = `
    <div style="overflow-x:auto">
      <table>
        <thead><tr><th style="width:24px">#</th><th>Centro (EESS)</th><th>Red</th><th style="text-align:right">Sin atender</th><th>Cobertura</th></tr></thead>
        <tbody>${rowsHtml || `<tr><td colspan="5" style="text-align:center;padding:16px;color:var(--muted)">Sin registros</td></tr>`}</tbody>
      </table>
    </div>`;
  showModal(prod, `${totalNoSat.toLocaleString('es-PE')} u sin atender en total · ${filas.length} centro${filas.length!==1?'s':''} reportan`, body);
}

// ═══════════════════════════════════════════════════════════════
// TOP ESTABLECIMIENTOS — mayor cantidad de productos no atendidos
// ═══════════════════════════════════════════════════════════════
function renderTopEstabTab(data) {
  const listEl = document.getElementById('topestab-list');
  const cntEl  = document.getElementById('te-count');
  if (!listEl) return; // pestaña aún no está en el DOM

  const busq = (document.getElementById('te-search')?.value || '').toLowerCase();

  // Agrupar por establecimiento
  const byEstab = {};
  data.forEach(r => {
    if (!byEstab[r.estab]) byEstab[r.estab] = { noSat:0, disponible:0, requerida:0, red:r.redes, cod_pre:r.cod_pre, nProductos:new Set() };
    byEstab[r.estab].noSat      += r.noSat;
    byEstab[r.estab].disponible += r.disponible;
    byEstab[r.estab].requerida  += r.requerida;
    byEstab[r.estab].nProductos.add(r.producto);
  });

  let sorted = Object.entries(byEstab)
    .filter(([estab]) => !busq || estab.toLowerCase().includes(busq))
    .sort((a,b) => b[1].noSat - a[1].noSat);

  topEstabCache = sorted;
  const maxV = sorted[0]?.[1].noSat || 1;

  if (cntEl) cntEl.textContent = `${sorted.length.toLocaleString()} establecimientos`;

  if (!sorted.length) {
    listEl.innerHTML = '<p style="text-align:center;color:var(--muted);padding:30px;font-size:12px">Sin resultados</p>';
    return;
  }

  listEl.innerHTML = sorted.map(([estab, info], i) => {
    const cob = info.requerida ? (info.disponible/info.requerida*100) : 0;
    const pct = Math.round((info.noSat/maxV)*100);
    const rankClass = i===0?'r1':i===1?'r2':i===2?'r3':'';
    const isOpen = topEstabOpen.has(i);
    // Detalle de productos de este establecimiento, de mayor a menor
    const porProducto = {};
    data.filter(r => r.estab === estab).forEach(r => {
      if (!porProducto[r.producto]) porProducto[r.producto] = { noSat:0, requerida:0, disponible:0, cod_sismed:r.cod_sismed, servicio:r.servicio };
      porProducto[r.producto].noSat      += r.noSat;
      porProducto[r.producto].requerida  += r.requerida;
      porProducto[r.producto].disponible += r.disponible;
    });
    const prodsOrdenados = Object.entries(porProducto).sort((a,b) => b[1].noSat - a[1].noSat);
    const detalleRows = prodsOrdenados.map(([prod, pv]) => {
      const pcob = pv.requerida ? (pv.disponible/pv.requerida*100) : 0;
      const pillCls = pcob===0?'pill-red':pcob<30?'pill-orange':pcob<100?'pill-yellow':'pill-green';
      return `<tr>
        <td class="mono" style="color:var(--muted2)">${pv.cod_sismed||'—'}</td>
        <td style="max-width:260px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="${prod}">${prod}</td>
        <td style="color:var(--muted2)">${pv.servicio||''}</td>
        <td class="mono" style="text-align:right;font-weight:700;color:var(--red)">${pv.noSat.toLocaleString()}</td>
        <td class="mono" style="text-align:right;color:var(--muted2)">${pv.requerida.toLocaleString()}</td>
        <td><span class="pill ${pillCls}">${pcob.toFixed(1)}%</span></td>
      </tr>`;
    }).join('');

    return `<div class="topestab-item ${isOpen?'open':''}">
      <div class="topestab-hdr" onclick="toggleTopEstab(${i})">
        <div class="rank ${rankClass}">${i+1}</div>
        <div class="topestab-info">
          <div class="topestab-name">${estab}</div>
          <div class="topestab-meta">${info.red||'Sin red'} · ${info.nProductos.size} producto${info.nProductos.size!==1?'s':''} distintos · Cobertura ${cob.toFixed(1)}%</div>
        </div>
        <div class="topestab-bar-wrap"><div class="nosat-bar-track" style="width:100%"><div class="nosat-bar-fill" style="width:${pct}%"></div></div></div>
        <div class="topestab-nosat">${info.noSat.toLocaleString('es-PE')} u</div>
        <div class="topestab-chevron">▶</div>
      </div>
      <div class="topestab-detail">
        <table>
          <thead><tr><th>COD SISMED</th><th>Producto</th><th>Servicio</th><th style="text-align:right">Sin atender</th><th style="text-align:right">Requerida</th><th>Cobertura</th></tr></thead>
          <tbody>${detalleRows || `<tr><td colspan="6" style="text-align:center;padding:16px;color:var(--muted)">Sin productos</td></tr>`}</tbody>
        </table>
      </div>
    </div>`;
  }).join('');
...
function toggleTopEstab(i) {
  if (topEstabOpen.has(i)) topEstabOpen.delete(i); else topEstabOpen.add(i);
  renderTopEstabTab(filteredData);
}
...
function populateFilters() {
  ...
  const sm = document.getElementById('t-mes');
  if (!se||!ss) return;

  // Poblar el selector de mes una sola vez, con TODOS los meses disponibles
  if (sm && sm.options.length <= 1) {
    const meses = [...new Set(allData.filter(r=>r.mesKey && r.mesKey!=='Sin fecha').map(r=>r.mesKey))].sort().reverse();
    meses.forEach(k => { const o=document.createElement('option'); o.value=k; o.textContent=mesLabel(k); sm.appendChild(o); });
  }
  const filtMes = sm?.value || '';
  // El resto de selects (EESS, servicio, red) se calculan sólo con lo que corresponde
  // al período activo (pills superiores) y, si se eligió, al mes de esta pestaña —
  // así solo aparecen los establecimientos que realmente tienen registros ahí.
  const scope = filtMes ? filteredData.filter(r => r.mesKey === filtMes) : filteredData;

  const estabs    = [...new Set(scope.map(r=>r.estab))].sort();
  const servicios = [...new Set(scope.map(r=>r.servicio))].sort();
  const pe=se.value, ps=ss.value;
  se.innerHTML = '<option value="">Todos los EESS</option>' + estabs.map(e=>`<option value="${e}">${e}</option>`).join('');
  const redes = [...new Set(scope.map(r=>r.redes))].sort();
  const sr = document.getElementById('t-redes');
  if (sr) { const pr=sr.value; sr.innerHTML = '<option value="">Todas las redes</option>' + redes.map(r=>`<option value="${r}">${r}</option>`).join(''); sr.value=pr; }
  ss.innerHTML = '<option value="">Todos los servicios</option>' + servicios.map(s=>`<option value="${s}">${s}</option>`).join('');
  se.value = estabs.includes(pe) ? pe : '';
  ss.value = servicios.includes(ps) ? ps : '';
}
...
function filterTable() {
  ...
  const fm = document.getElementById('t-mes')?.value||'';
